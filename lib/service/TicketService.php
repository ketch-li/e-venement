...
class TicketService extends EvenementService
{
  ...
  public function singleControlDemat($user, $event_id, $ticket_code)
  {
    ...
    if ( !$ticket )
      throw new liApiNotFoundException("Ticket not found");
      
    // Check if ticket is integrated
    if ( !$ticket->integrated_at )
      throw new liApiNotFoundException("The ticket has not been printed");
      
    // Check if ticket manifestation matches event_id
    if ( $ticket->Manifestation->event_id != $event_id )
      throw new liApiBadRequestException("The ticket manifestation does not match the event id #" . $event_id);
      
    $checkpoint = null;
    // Get the checkpoint of the event
    if ( $ticket->Manifestation->Event->Checkpoints->count() == 0 )
      throw new liApiNotFoundException("No ENTRANCE checkpoint configured for event #" . $event_id);
    else
    ...
    if ( $ticket->Manifestation->happens_at > date('Y-m-d H:i', strtotime('now + ' . $future)) )
      throw new liApiNotFoundException("The control for manifestation #" . $ticket->Manifestation->id . " is not available yet");
    ...
    if ( strtotime($ticket->Manifestation->ends_at) < strtotime('now - '.$past) && !$post )
      throw new liApiNotFoundException("The control for manifestation #" . $ticket->Manifestation->id . " is over");
    ...
    if ( $ticket->Controls->count() > 0 )
    {
      ...
      throw new liApiNotFoundException("The ticket has already been controled");
    }
