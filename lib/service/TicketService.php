...
class TicketService extends EvenementService
{
  ...
  {
    if ( !$ticket )
      throw new Exception(__("Ticket not found", null, 'ticket_service'), 1001);
      
    // Check if ticket is integrated
    if ( !$ticket->integrated_at )
      throw new Exception(__("The ticket has not been printed", null, 'ticket_service'), 1002);
  }
  ...
  protected function controlDemat($user_id, $event_id, Ticket $ticket, $post)
  {
    ...
    if ( $ticket->Manifestation->event_id != $event_id )
      throw new Exception(__("The ticket manifestation does not match the event id #%%EVENT_ID%%", array('%%EVENT_ID%%' => $event_id), 'ticket_service'), 1003);
    ...
    if ( $checkpoint->event_id != $event_id )
      throw new Exception(__("No ENTRANCE checkpoint configured for event #%%EVENT_ID%%", array('%%EVENT_ID%%', $event_id), 'ticket_service'), 1004);
    ...
    if ( $ticket->Manifestation->happens_at > $start )
      throw new Exception(__("The control for manifestation #%%MANIFESTATION_ID%% is not available yet", array('%%MANIFESTATION_ID%%' => $ticket->Manifestation->id), 'ticket_service'), 1005);
      
    if ( $ticket->Manifestation->happens_at < $start && strtotime($ticket->Manifestation->ends_at) > $end && $post ) {
      throw new Exception(__("The manifestation #%%MANIFESTATION_ID%% is not over yet", array('%%MANIFESTATION_ID%%' => $ticket->Manifestation->id), 'ticket_service'), 1008);
    }
      
    // Check if the manifestation is over to allow control after the end
    if ( strtotime($ticket->Manifestation->ends_at) < $end && !$post )
      throw new Exception(__("The control for manifestation #%%MANIFESTATION_ID%% is over", array('%%MANIFESTATION_ID%%' => $ticket->Manifestation->id), 'ticket_service'), 1006);
    ...
    if ( $ticket->Controls->count() > 0 )
    {
      ...
      throw new Exception(__("The ticket #%%TICKET_ID%% has already been controlled", array('%%TICKET_ID%%' => $ticket->id), 'ticket_service'), 1007);
    }
