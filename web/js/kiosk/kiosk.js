		 ...
		 .then(function() {
		 	$.get(
				LI.kiosk.urls.printTickets.replace('-666', LI.kiosk.transaction.id) +
					...
					', "pid": ' + 
					LI.kiosk.devices.ticketPrinter.params.pid + 
					'}'
				, 
				function(data) {
		            //console.log(data);
		            ...
		            LI.kiosk.connector.sendData(LI.kiosk.devices.ticketPrinter, data).then(
			            function(res) {
			            	console.log(res);
			            	LI.kiosk.connector.readData(LI.kiosk.devices.ticketPrinter).then(function(response) {
