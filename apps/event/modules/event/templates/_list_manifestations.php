...
<?php foreach ( $event->Manifestations as $manif ): ?>
  <?php
    $manifs[$manif->happens_at.'-'.$manif->id] =
      ...
      ).' <a class="sell" title="'.__('Sell').'" href="'.cross_app_url_for('tck','ticket/sell?hash='.$manif->id.'&type='.sfContext::getInstance()->getConfiguration()->getApplication().'#manif-'.$manif->id).'"><span class="ui-icon-li ui-icon ui-icon-microplus"></span></a>'
  ?>
<?php endforeach ?>
<?php
