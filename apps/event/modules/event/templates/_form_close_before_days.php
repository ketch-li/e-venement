...
$(document).ready(function(){
  $('.sf_admin_form_field_close_before input').change(function(){
  ...
  $('.sf_admin_form_field_close_before_days input').change(function(){
    var hrs_close = parseInt($('.sf_admin_form_field_close_before input').val().replace(/:\d\d$/, ''))%24 + $(this).val()*24;
    var mins_close = $('.sf_admin_form_field_close_before input').val().replace(/^-{0,1}\d+:/, '');
    $('.sf_admin_form_field_close_before input').val(hrs_close+':'+mins_close);
  });
});
--></script>
</div>
