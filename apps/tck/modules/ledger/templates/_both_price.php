...
<?php foreach ( $byPrice as $price ): ?>
  <tr class="<?php echo ($class = !$class) ? 'overlined' : '' ?>">
    ...
    <td class="incomes amount"><?php echo format_currency($i,$sf_context->getConfiguration()->getCurrency()); $total['value+'] += $i; ?></td>
    <td class="nb"><?php echo $price->nb_tickets-$c*2; ?></td>
    <td class="total"><?php echo format_currency($i+$o,$sf_context->getConfiguration()->getCurrency()); ?></td>
  </tr>
<?php endforeach ?>
</tbody>
<tfoot>
