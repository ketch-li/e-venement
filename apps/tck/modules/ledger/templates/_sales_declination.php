...
<td class="value"><?php echo format_currency($declination['value'], $sf_context->getConfiguration()->getCurrency()) ?></td>
<td class="extra-taxes"><?php echo format_currency($declination['taxes'], $sf_context->getConfiguration()->getCurrency()) ?></td>
<?php $local_vat = 0 ?>
<?php foreach ( $vat as $t ): if ( $t[$pdtname][$dname]['__total__'] ): ?>
  <?php $local_vat += round($t[$pdtname][$dname]['__total__'],2) ?>
  <td class="vat"><?php echo format_currency(round($t[$pdtname][$dname]['__total__'],2),$sf_context->getConfiguration()->getCurrency()) ?></td>
<?php else: ?>
<td class="vat"></td>
<?php endif; endforeach ?>
<td class="vat total"><?php echo format_currency($local_vat,$sf_context->getConfiguration()->getCurrency()) ?></td>
<td class="tep"><?php echo format_currency($declination['value'] - $local_vat,$sf_context->getConfiguration()->getCurrency()) ?></td>
